...
const elementos = {
    ...
    boneco: {
        cabeca: document.querySelector('.boneco-cabeca'),
        corpo: document.querySelector('.boneco-corpo'),
        bracoEsquerdo: document.querySelector('.boneco-braco-esquerdo'),
        bracoDireito: document.querySelector('.boneco-braco-direito'),
        pernaEsquerda: document.querySelector('.boneco-perna-esquerda'),
        pernaDireita: document.querySelector('.boneco-perna-direita'),
    }
};
...
function cadastrarPalavra() {
    ...
    if (!novaPalavra || !novaDica) {
        alert('Por favor, preencha todos os campos...');
        return;
    }

    const novaPalavraObjeto = {
        palavra: novaPalavra,
        dica: novaDica,
    };

    palavras[dificuldadeSelecionada].push(novaPalavraObjeto);

    elementos.campos.palavra.value = '';
    elementos.campos.dica.value = '';

    alert('Obaaa!!! Palavra cadastrada com sucesso!');
...
function novoJogo() {
    ...
    return novoJogoObjeto;
}

function definirPalavra(palavra, dica) {
...
function jogar(letraJogada) {
    let acertou = false;
    for (let i = 0; i < jogo.palavra.tamanho; i++) {
        const letra = jogo.palavra.semAcentos[i].toLowerCase();
